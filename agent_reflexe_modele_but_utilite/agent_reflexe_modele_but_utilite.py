...
def va_vers_la_poussiere(a, b):
    # On va chercher, parmis les 2 pièces données en paramètre, 1 pièce salle
    # Le bot va alors se diriger vers celle-ci (voir méthode plus bas)
...
def get_resume_agent():
    ...
    # On affiche également l'efficacité de l'agent suite à l'évolution
    if nombre_pieces_salles != 0:
        efficacite = round((perception.count(ASPIRER_SALLE) / nombre_pieces_salles) * 100, 2)
        print("> Il y'avait {} pièces sales au départ, j'ai donc nettoyé {}% des pièces\n{}".format(
            perception.count(ASPIRER_SALLE),
            efficacite,
            "> Quelle efficacité !" if efficacite > 0.6 else "> On peut mieux faire ..."
        ))
    else:
        print("> Il n'y a eu aucune pièce sale à nettoyer, l'agent s'ennuie ...")
